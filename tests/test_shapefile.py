...
def test_reader_geo_interface(pyshp_shapefiles):
    with shapefile.Reader(join(pyshp_shapefiles, "blockgroups")) as r:
...
def test_shapes_geo_interface(pyshp_shapefiles):
    with shapefile.Reader(join(pyshp_shapefiles, "blockgroups")) as r:
...
def test_shaperecords_geo_interface(pyshp_shapefiles):
    with shapefile.Reader(join(pyshp_shapefiles, "blockgroups")) as r:
...
def test_shaperecord_geo_interface(pyshp_shapefiles):
    with shapefile.Reader(join(pyshp_shapefiles, "blockgroups")) as r:
        for shaperec in r:
...
def test_reader_zip(pyshp_shapefiles):
    """
    Assert that Reader can open shapefiles inside a zipfile.
    """
    # test reading zipfile only
    with shapefile.Reader(join(pyshp_shapefiles, "blockgroups.zip")) as sf:
        for recShape in sf.iterShapeRecords():
            pass
        assert len(sf) > 0
    assert sf.shp.closed == sf.shx.closed == sf.dbf.closed == True
    ...
    with pytest.raises(shapefile.ShapefileException):
        with shapefile.Reader(join(pyshp_shapefiles, "blockgroups_multishapefile.zip")) as sf:
            pass

    # test specifying the path when reading multi-shapefile zipfile (with extension)
    with shapefile.Reader(join(pyshp_shapefiles, "blockgroups_multishapefile.zip/blockgroups2.shp")) as sf:
        for recShape in sf.iterShapeRecords():
            pass
        assert len(sf) > 0
    assert sf.shp.closed == sf.shx.closed == sf.dbf.closed == True
    ...
    with shapefile.Reader(join(pyshp_shapefiles, "blockgroups_multishapefile.zip/blockgroups2")) as sf:
        for recShape in sf.iterShapeRecords():
            pass
        assert len(sf) > 0
    assert sf.shp.closed == sf.shx.closed == sf.dbf.closed == True
    ...
    with pytest.raises(shapefile.ShapefileException):
        with shapefile.Reader(join(pyshp_shapefiles, "empty_zipfile.zip")) as sf:
            pass


def test_reader_close_path(pyshp_shapefiles):
    """
    ...
    sf = shapefile.Reader(join(pyshp_shapefiles, "blockgroups.shp"))
    sf.close()
...
def test_reader_close_filelike(pyshp_shapefiles):
    """
    Assert that manually calling Reader.close()
    ...
    # note uses an actual shapefile from
    # the projects "shapefiles" directory
    shp = open(join(pyshp_shapefiles, "blockgroups.shp"), mode='rb')
    shx = open(join(pyshp_shapefiles, "blockgroups.shx"), mode='rb')
    dbf = open(join(pyshp_shapefiles, "blockgroups.dbf"), mode='rb')
...
def test_reader_context_path(pyshp_shapefiles):
    """
    ...
    # note uses an actual shapefile from
    # the projects "shapefiles" directory
    with shapefile.Reader(join(pyshp_shapefiles, "blockgroups")) as sf:
        pass
...
def test_reader_context_filelike(pyshp_shapefiles):
    """
    Assert that using the context manager
    leaves the shp, shx, and dbf files open
    on exit, if given filelike objects.
...
def test_reader_shapefile_type(pyshp_shapefiles):
    """
...
def test_reader_shapefile_length(pyshp_shapefiles):
    """
...
def test_shape_metadata(pyshp_shapefiles):
    with shapefile.Reader(join(pyshp_shapefiles, "blockgroups")) as sf:
        shape = sf.shape(0)
...
def test_reader_fields(pyshp_shapefiles):
    """
...
def test_reader_shapefile_extension_ignored(pyshp_shapefiles):
    """
    Assert that the filename's extension is
    ignored when reading a shapefile.
    """
    base = join(pyshp_shapefiles, "blockgroups")
    ext = ".abc"
    filename = base + ext
...
def test_reader_pathlike(pyshp_shapefiles):
    """
    Assert that path-like objects can be read.
    """
    base = Path(pyshp_shapefiles)
    with shapefile.Reader(base / "blockgroups") as sf:
        assert len(sf) == 663


def test_reader_dbf_only(pyshp_shapefiles):
    """
    Assert that specifying just the
    dbf argument to the shapefile reader
    reads just the dbf file.
    """
    with shapefile.Reader(dbf=join(pyshp_shapefiles, "blockgroups.dbf")) as sf:
        assert len(sf) == 663
        record = sf.record(3)
        assert record[1:3] == ['060750601001', 4715]


def test_reader_shp_shx_only(pyshp_shapefiles):
    """
    Assert that specifying just the
    shp and shx argument to the shapefile reader
    reads just the shp and shx file.
    """
    with shapefile.Reader(shp=join(pyshp_shapefiles, "blockgroups.shp"), shx=join(pyshp_shapefiles, "blockgroups.shx")) as sf:
        assert len(sf) == 663
        shape = sf.shape(3)
        assert len(shape.points) is 173

    
def test_reader_shp_dbf_only(pyshp_shapefiles):
    """
    Assert that specifying just the
    shp and shx argument to the shapefile reader
    reads just the shp and dbf file.
    """
    with shapefile.Reader(shp=join(pyshp_shapefiles, "blockgroups.shp"), dbf=join(pyshp_shapefiles, "blockgroups.dbf")) as sf:
        assert len(sf) == 663
        shape = sf.shape(3)
        assert len(shape.points) is 173
        record = sf.record(3)
...
def test_reader_shp_only(pyshp_shapefiles):
    """
    Assert that specifying just the
    shp argument to the shapefile reader
    reads just the shp file (shx optional).
    """
    with shapefile.Reader(shp=join(pyshp_shapefiles, "blockgroups.shp")) as sf:
        assert len(sf) == 663
        shape = sf.shape(3)
        assert len(shape.points) is 173


def test_reader_filelike_dbf_only(pyshp_shapefiles):
    """
    Assert that specifying just the
    dbf argument to the shapefile reader
    reads just the dbf file.
    """
    with shapefile.Reader(dbf=open(join(pyshp_shapefiles, "blockgroups.dbf"), "rb")) as sf:
        assert len(sf) == 663
        record = sf.record(3)
        assert record[1:3] == ['060750601001', 4715]


def test_reader_filelike_shp_shx_only(pyshp_shapefiles):
    """
    Assert that specifying just the
    shp and shx argument to the shapefile reader
    reads just the shp and shx file.
    """
    with shapefile.Reader(shp=open(join(pyshp_shapefiles, "blockgroups.shp"), "rb"), shx=open(join(pyshp_shapefiles, "blockgroups.shx"), "rb")) as sf:
        assert len(sf) == 663
        shape = sf.shape(3)
        assert len(shape.points) is 173


def test_reader_filelike_shp_dbf_only(pyshp_shapefiles):
    """
    Assert that specifying just the
    shp and shx argument to the shapefile reader
    reads just the shp and dbf file.
    """
    with shapefile.Reader(shp=open(join(pyshp_shapefiles, "blockgroups.shp"), "rb"), dbf=open(join(pyshp_shapefiles, "blockgroups.dbf"), "rb")) as sf:
        assert len(sf) == 663
        shape = sf.shape(3)
        assert len(shape.points) is 173
        record = sf.record(3)
...
def test_reader_filelike_shp_only(pyshp_shapefiles):
    """
    Assert that specifying just the
    shp argument to the shapefile reader
    reads just the shp file (shx optional).
    """
    with shapefile.Reader(shp=open(join(pyshp_shapefiles, "blockgroups.shp"), "rb")) as sf:
        assert len(sf) == 663
        shape = sf.shape(3)
        assert len(shape.points) is 173


def test_reader_shapefile_delayed_load(pyshp_shapefiles):
    """
    Assert that the filename's extension is
    ignored when reading a shapefile.
    """
    with shapefile.Reader() as sf:
        ...
        sf.load(join(pyshp_shapefiles, "blockgroups"))
        assert len(sf) == 663


def test_records_match_shapes(pyshp_shapefiles):
    """
...
def test_record_attributes(pyshp_shapefiles, fields=None):
    """
...
def test_record_subfields(pyshp_shapefiles):
    """
    Assert that reader correctly retrieves only a subset 
    of fields when specified.
    """
    fields = ["AREA","POP1990","MALES","FEMALES","MOBILEHOME"]
    test_record_attributes(pyshp_shapefiles, fields=fields)


def test_record_subfields_unordered(pyshp_shapefiles):
    """
    Assert that reader correctly retrieves only a subset 
...
def test_record_subfields_delflag_notvalid(pyshp_shapefiles):
    """
...
def test_record_subfields_duplicates(pyshp_shapefiles):
    """
    Assert that reader correctly retrieves only a subset
...
def test_record_subfields_empty(pyshp_shapefiles):
    """
...
def test_record_as_dict(pyshp_shapefiles):
    """
...
def test_record_oid(pyshp_shapefiles):
    """
...
def test_shape_oid(pyshp_shapefiles):
    """
...
def test_shape_oid_no_shx(pyshp_shapefiles):
    """
    Assert that the shape's oid attribute returns
    its index in the shapefile, when shx file is missing.
    """
    basename = join(pyshp_shapefiles, "blockgroups")
...
def test_reader_offsets(pyshp_shapefiles):
    """
...
def test_reader_offsets_no_shx(pyshp_shapefiles):
    """
...
def test_reader_numshapes(pyshp_shapefiles):
    """
...
def test_reader_numshapes_no_shx(pyshp_shapefiles):
    """
...
def test_reader_len(pyshp_shapefiles):
    """
...
def test_reader_len_dbf_only(pyshp_shapefiles):
    """
...
def test_reader_len_no_dbf(pyshp_shapefiles):
    """
...
def test_reader_len_no_dbf_shx(pyshp_shapefiles):
    """
...
def test_reader_corrupt_files(pyshp_shapefiles):
    """
    Assert that reader is able to handle corrupt files by
    strictly going off the header information. 
    """
    basename = join(pyshp_shapefiles, "test/corrupt_too_long")
...
def test_bboxfilter_shapes(pyshp_shapefiles):
    """
...
def test_bboxfilter_shapes_outside(pyshp_shapefiles):
    """
...
def test_bboxfilter_itershapes(pyshp_shapefiles):
    """
...
def test_bboxfilter_shaperecord(pyshp_shapefiles):
    """
...
def test_bboxfilter_shaperecords(pyshp_shapefiles):
    """
...
def test_bboxfilter_itershaperecords(pyshp_shapefiles):
    """
...
def test_shaperecords_shaperecord(pyshp_shapefiles):
    """
...
def test_shaperecord_shape(pyshp_shapefiles):
    """
...
def test_shaperecord_record(pyshp_shapefiles):
    """
