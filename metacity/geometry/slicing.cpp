...
class TriangleSlicer
{
    ...
    pair<int, int> setup_range(const tvec3 t[3], tfloat tile_size, size_t axis)
    {
        pair<tfloat, tfloat> range;
        range.first = min(t[axis].x, min(t[axis].x, t[axis].x));
        range.second = max(t[axis].x, max(t[axis].x, t[axis].x));
    ...
    tvec3 cross_point(const tvec3 &a, const tvec3 &b, const K::Plane_3 plane)
    {
    ...
    bool not_splitable(const tvec3 t[3], K::Plane_3 plane, size_t axis)
    {
        auto p = plane.point()[axis];
        return ((t[0][axis] <= p) && (t[1][axis] <= p) && (t[2][axis] <= p)) ||
               ((t[0][axis] >= p) && (t[1][axis] >= p) && (t[2][axis] >= p));
    }

    bool point_on_plane(const tvec3 t[3], K::Plane_3 plane, size_t axis)
    {
        auto p = plane.point()[axis];
        return (t[0][axis] == p) || (t[1][axis] == p) || (t[2][axis] == p);
    }

    void one_on_plane_split(const tvec3 t[3], K::Plane_3 plane)
    {
    ...
    void special_case_split(const tvec3 t[3], K::Plane_3 plane, size_t axis)
    {
        auto p = plane.point()[axis];
        bool aop = (t[0][axis] == p);
        bool bop = (t[1][axis] == p);
    ...
    inline bool bc_on_same_side(const tvec3 t[3], tfloat p, size_t axis)
    {
        bool b = (t[1][axis] < p);
        bool c = (t[2][axis] < p);
        return (b && c) || !(b || c);
    ...
    void orinet(tvec3 t[3], tfloat p, size_t axis)
    {
        size_t i = 0;
    ...
    void general_case_split(const tvec3 t[3], K::Plane_3 plane, size_t axis)
    {
    ...
    void split_triangle_along_axis(const tvec3 triangle[3], K::Plane_3 plane, size_t axis)
    {
        if (not_splitable(triangle, plane, axis))
            return insert_triangle_split(triangle);

        if (point_on_plane(triangle, plane, axis))
            special_case_split(triangle, plane, axis);
        else
            general_case_split(triangle, plane, axis);
    }

    void split_triangles_along_axis(const pair<int, int> &range, size_t axis)
    {
        tvec3 origin(0);
        for (int i = range.first; i <= range.second; ++i)
        {
            ...
            for (size_t i = 0; i < triangles.size(); i += 3)
                split_triangle_along_axis(&triangles[i], plane, axis);
            triangles = splited;
        }
    }
